...
class ACertificateReleasingComponent extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        $training_schedules = tblcourseschedule::addSelect([
            ...
            'pending_releasing_count' => tblenroled::select(DB::raw('COUNT(*)'))
                ->join('tblcertificatehistory', 'tblcertificatehistory.enroledid', '=', 'tblenroled.enroledid')
                ->whereColumn('tblcourseschedule.scheduleid', 'tblenroled.scheduleid')
                ->where(function ($query) {
                    $query->where('tblcertificatehistory.is_approve', 1)
            ...
            ->having('pending_releasing_count', '>', 0) // Exclude schedules with enrolled_pending_count = 0
            ->where('printedid', 1);
        ...
        return view(
            'livewire.admin.approval.releasing.a-certificate-releasing-component',
            [
