...
class TGenerateSalaryDeducComponent extends Component
{
    public $enrol;
    public $meal_price;
    public $registration;
    ...
    {     
        ...
        $sample = tblenroled::where('enroledid', $this->registration)->first();
        if ($sample === null) {
        ...
        $filePath = public_path("assets/template/atd_solo_nyk.pdf");
    ...
    public function fillPDFFile($file, $outputFilePath)
    {
        ...
        for ($i = 1; $i <= $count; $i++) {
            ...
            $left = 160;
            $top = 95.5;
            $duration = $checkin1->diffInDays($checkout1);
            ...
            $total_dorm_meal = ($this->enrol->dorm_price != 0 && $this->enrol->meal_price != 0)
                ? number_format(($this->enrol->dorm_price) + ($this->enrol->meal_price))
                : 'N/A';
    ...
    {
        return view('livewire.trainee.enroll.t-generate-salary-deduc-component');
    }
}
